...
function ConvertTo-ScopeTokens {
    param([string]$Text)
...
if ((ConvertTo-ScopeTokens $actual) -cne (ConvertTo-ScopeTokens $expected)) { throw 'Unexpected queue behavior beyond completion accounting.' }
...
if ((ConvertTo-ScopeTokens $actual) -cne (ConvertTo-ScopeTokens $expected)) { throw 'Unexpected queue layout/limits.' }
...
if ((ConvertTo-ScopeTokens (Get-ScopeSource 'src/driver/driver.h')) -cne (ConvertTo-ScopeTokens $expected)) { throw 'Unexpected adapter layout or queue cap.' }
...
if ((ConvertTo-ScopeTokens $actual) -cne (ConvertTo-ScopeTokens $expected)) { throw 'Unexpected driver-core behavior outside diagnostics/version.' }
