...
if (Test-Path -LiteralPath $stage) { throw 'Diagnostics staging directory already exists; use a clean CI checkout.' }
...
Copy-Item -LiteralPath (Join-Path $root 'utility\RPi5-WiFi-Operations.ps1') -Destination (Join-Path $stage 'RPi5-WiFi-Operations.ps1') -Force
Copy-Item -LiteralPath (Join-Path $root 'utility\Get-RPi5-WiFi-Transport.ps1') -Destination (Join-Path $stage 'Get-RPi5-WiFi-Transport.ps1') -Force

@"
repository=$env:GITHUB_REPOSITORY
commit=$env:GITHUB_SHA
workflow_run=$env:GITHUB_SERVER_URL/$env:GITHUB_REPOSITORY/actions/runs/$env:GITHUB_RUN_ID
utility_version=0.6.26
